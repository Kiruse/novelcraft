...
pub(crate) async fn ensure_dir(path: &Path) -> Result<(), AppError> {
  tokio::fs::create_dir_all(path)
    .await
    .map_err(|e| AppError::io(format!("Failed to create directory: {}", e)))
}

pub(crate) async fn deserialize<T: DeserializeOwned>(path: &Path) -> Result<T, AppError> {
...
pub(crate) async fn serialize(path: &Path, obj: &impl Serialize) -> Result<(), AppError> {
