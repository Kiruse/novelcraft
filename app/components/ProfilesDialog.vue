...
const focusedValueIdx = ref(-1);
const pendingDeleteId = ref<string | null>(null);
const valueRefs = ref<(HTMLInputElement | HTMLTextAreaElement)[]>([]);

function onValueFocus(_e: FocusEvent, idx: number) {
  focusedValueIdx.value = idx;
}

watch(() => {
  const fields = editFields.value;
  const idx = focusedValueIdx.value;
  if (idx === -1 || idx >= fields.length) return -1;
  return fields[idx]!.value.length;
}, () => {
  nextTick(() => {
    const el = valueRefs.value[focusedValueIdx.value];
    if (el) el.focus();
  });
});
