  ...
  <form class="builder-form" @submit.prevent="publish">
    ...
    <div v-if="result" class="result">
      <slot name="result" :result="result">
        <p>✅ <RouterLink :to="`/stories/${result.authorName}/${result.storyId}`">{{ result.title }}</RouterLink></p>
      </slot>
    </div>
  </form>
</template>
...
import BuilderInspireDialog from '~/components/builder/InspireDialog.vue';
...
async function navigateToTest() {
  ...
  await router.push(`/stories/${result.value.authorName}/${result.value.storyId}?test=1`);
}
