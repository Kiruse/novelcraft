...
- **Fork** — `NovelCraftEngine::fork(page_index)` deletes page batch files after the batch containing `page_index`, truncates the pages after `page_index` within its containing batch (rewriting that batch file), then reloads the session via `SessionV1::load` to obtain correct `tail_batches`, `page_count`, `batch_count`, and `gamestate`
